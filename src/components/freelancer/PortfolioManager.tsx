...
export default function PortfolioManager({ freelancerId }: PortfolioProps) {
  ...
  const [portfolioItems, setPortfolioItems] = useState<Freelancer['portfolio_items']>([]);
  ...
  const loadPortfolio = async () => {
    try {
      ...
      setPortfolioItems(freelancer.portfolio_items || []);
    } catch (error) {
  ...
  const handleEnhanceImages = async () => {
    try {
      setLoading(true);
      const result = await freelancerService.enhancePortfolioImages(freelancerId);
      toast.success(result.message);
      loadPortfolio();
    } catch (error) {
  ...
  return (
    <Card className="mb-8">
      <div className="flex justify-between items-center mb-4 px-4 py-2">
        ...
        <div>
          <Button onClick={handleSyncPortfolio} disabled={loading} variant="outline">
            Sync Portfolio
          </Button>
          <Button onClick={handleEnhanceImages} disabled={loading} className="ml-2">
            Enhance Images
          </Button>
        </div>
      </div>
